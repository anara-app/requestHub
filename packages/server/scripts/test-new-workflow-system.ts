...
async function testNewWorkflowSystem() {
  ...
  try {
    ...
    console.log(
      `   Manager: ${(testUser as any).manager ? `${(testUser as any).manager.firstName} ${(testUser as any).manager.lastName}` : "None"}\n`
    );
    ...
    for (const test of roleBasedTests) {
      const userId = await WorkflowAssignmentService.resolveRoleBasedAssignee(
        test.roleName
      );
      if (userId) {
        ...
        console.log(
          `   ✅ ${test.description}: ${user?.firstName} ${user?.lastName} (${user?.email})`
        );
      } else {
    ...
    const managerId = await WorkflowAssignmentService.resolveDynamicAssignee(
    ...
    if (managerId) {
      ...
      console.log(
        `   ✅ INITIATOR_SUPERVISOR: ${manager?.firstName} ${manager?.lastName} (${manager?.email})`
      );
    } else {
    ...
    for (const stepDef of stepDefinitions) {
      ...
      if (assigneeId) {
        ...
        const assigneeType =
          stepDef.assigneeType === "DYNAMIC"
            ? stepDef.dynamicAssignee
            : stepDef.roleBasedAssignee;
        console.log(
          `   ✅ ${assigneeType}: ${assignee?.firstName} ${assignee?.lastName} (${assignee?.email})`
        );
      } else {
        const assigneeType =
          stepDef.assigneeType === "DYNAMIC"
    ...
    // Test 5: Role conversion
    console.log("5️⃣ Testing role conversion...");
    const testRoles = [
      "INITIATOR_SUPERVISOR",
      "Ceo",
      "Finance_manager",
    ...
    for (const roleName of testRoles) {
      const stepDef = WorkflowAssignmentService.convertRoleToStepDefinition(
        roleName,
        `${roleName} Approval`
      );
      console.log(
        `   ${roleName} → ${stepDef.assigneeType === "DYNAMIC" ? `DYNAMIC(${stepDef.dynamicAssignee})` : `ROLE_BASED(${stepDef.roleBasedAssignee})`}`
      );
    }
